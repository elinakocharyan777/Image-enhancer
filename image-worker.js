...
async function processImage(message) {
  try {
    ...
    sendStatus(taskId, 30, "анализ пикселей изображения");
    ...
    sendStatus(taskId, 50, "подбор параметров коррекции");
    ...
    sendStatus(taskId, 60, "начало обработки изображения");
...
function predictCorrectionParams(features) {
  ...
  if (features.contrast < 45) {
    contrastValue = 1.25;
  } else if (features.contrast > 80) {
    contrastValue = 0.95;
  } else {
    contrastValue = 1.1;
  }

  if (features.averageSaturation < 0.35) {
    saturationValue = 1.25;
  } else if (features.averageSaturation > 0.7) {
